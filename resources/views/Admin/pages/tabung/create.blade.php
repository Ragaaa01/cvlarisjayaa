...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Tambah Tabung</h6>
    </div>
    <div class="card-body">
        @if ($errors->any())
        ...
        <form action="{{ route('admin.tabung.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="kode_tabung" class="form-control" value="{{ old('kode_tabung') }}" required>
            </div>
            <div class="form-group">
                ...
                <select name="id_jenis_tabung" class="form-control select2" required>
                    ...
                    @foreach ($jenisTabungs as $jenis)
                        <option value="{{ $jenis->id_jenis_tabung }}" {{ old('id_jenis_tabung') == $jenis->id_jenis_tabung ? 'selected' : '' }}>{{ $jenis->nama_jenis }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="id_status_tabung" class="form-control select2" required>
                    ...
                    @foreach ($statusTabungs as $status)
                        <option value="{{ $status->id_status_tabung }}" {{ old('id_status_tabung') == $status->id_status_tabung ? 'selected' : '' }}>{{ $status->status_tabung }}</option>
                    @endforeach
                </select>
            </div>
